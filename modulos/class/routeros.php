...
public function conectar()
{
  ...
  $parametros = $mysqli->get_routeros_radius();
  $response = self::connect($parametros['ip'], $parametros['user'], $parametros['pass']);
   return $response;
}

public function print_sessions (){

  $con_api  = self::conectar();
  if ($con_api) {
    $comando = self::write('/tool/user-manager/session/print',false);
    $comando = self::write('=where=', false);
    $comando = self::write('?status=interim', true);
    $response = self::read(false);
    $array_response = self::parseResponse($response);
...
public function print_sessions_search ($s){

  $con_api  = self::conectar();
  if ($con_api) {
    $comando = self::write('/tool/user-manager/session/print',false);
    $comando = self::write('=where=', false);
    $comando = self::write('?user='.$s.'', true);
    $response = self::read(false);
    $array_response = self::parseResponse($response);
...
public function add_client_routeros ($user, $password){
  $con_api  = self::conectar();
  if ($con_api){
    $exec_add = self::comm("/tool/user-manager/user/add", array(
              "customer"   => "admin",
              "username"   => "$user",
              "password" => "$password",
...
public function activar_client_routeros ($user, $plan){
  $con_api  = self::conectar();
  if ($con_api){
    $exec_add = self::comm("/tool/user-manager/user/create-and-activate-profile", array(
              "customer"   => "admin",
              "numbers" => "$user",
               "customer" => "admin",
               "profile" => "$plan",
